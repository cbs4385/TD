    ...
    */
    ...
    public class DevouringMawEffect : ActivePowerEffect
    {
        ...
        private void UpdateDevourCycle()
        {
            ...
            switch (currentPhase)
            {
                case DevourPhase.Emerging:
                    ...
                    if (emergeT >= 1f)
                    {
                        Debug.Log($"[DevouringMaw] Emerge complete, transitioning to Paused. elapsedTime={elapsedTime}");
                ...
                case DevourPhase.Paused:
                    ...
                    {
                        Debug.Log($"[DevouringMaw] Pause complete after {phaseElapsed}s (required {PAUSE_DURATION}s), transitioning to Sinking. Visitors to devour: {visitorsBeingDevoured.Count}");
        ...
        private void InstantiateDevourVisual(Vector3 position)
        {
            ...
            Debug.Log($"[DevouringMaw] Instantiating devour visual at {position}");
            ...
            Debug.Log($"[DevouringMaw] GetComponent<Animator> returned: {(devourAnimator != null ? "found" : "null")}");

            if (devourAnimator == null)
            {
                devourAnimator = devourVisual.GetComponentInChildren<Animator>();
                Debug.Log($"[DevouringMaw] GetComponentInChildren<Animator> returned: {(devourAnimator != null ? "found" : "null")}");
            }
            ...
            if (devourAnimator != null)
            {
                ...
                if (controller == null)
                {
                    ...
                    if (controller != null)
                    {
                        ...
                        Debug.Log($"[DevouringMaw] Loaded and assigned animator controller: {controller.name}");
                    }
#endif
                }
                ...
                if (controller != null)
                {
                    ...
                    devourAnimator.Update(0f);
                    Debug.Log($"[DevouringMaw] Animator set up at frame 1, normalizedTime={startNormalized}");
                }
                else
                {
        ...
        private void ConsumeVisitor(VisitorControllerBase visitor)
        {
            if (visitor == null)
            {
                Debug.Log("[DevouringMaw] ConsumeVisitor called with null visitor");
                return;
            }
            ...
            Debug.Log($"[DevouringMaw] Consuming visitor, base essence: {baseEssence}, awarding: {essence}");

            if (manager.GameController != null)
            {
                manager.GameController.AddEssence(essence);
                Debug.Log($"[DevouringMaw] Added {essence} essence via GameController");
            }
            else
            {
